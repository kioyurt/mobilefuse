    ...
    if hasattr(vae, "enable_tiling"):
        try:
            vae.enable_tiling(
                ...
                tile_sample_stride_height=0.5,
                tile_sample_stride_width=0.5,
            )
            logger.info("VAE tiled decode 已启用")
        except TypeError:
            vae.enable_tiling()
            logger.info("VAE tiled decode 已启用（默认参数）")
    scaling_factor = float(getattr(vae.config, "scaling_factor", 1.0))
    _log_cuda_memory("models_loaded")
    ...
    while global_step < total_steps:
        ...
        for sample in loader:
            ...
            decoded = torch.utils.checkpoint.checkpoint(
                ...
                (x1_pixel / scaling_factor).to(torch.float16),
                use_reentrant=False,
            )
...
def parse_args():
    ...
    p.add_argument("--pixel_latent_size", type=int, default=4,
                    choices=[2, 4, 8, 16],
                    help="训练时像素损失使用的 latent 边长；4 对应约 128x128 解码，显存最省")
