...
# 必须在 import torch 前设置，减少 CUDA 内存碎片。
if os.name != "nt":
...
class TrainableFusionConditioner(nn.Module):
    ...
    def forward(
        ...
        if getattr(self, "use_mcp_checkpoint", False) and torch.is_grad_enabled():
            # non-reentrant checkpoint avoids storing the large Mamba intermediate
            # activations. Rebuild the sparse hidden tuple during backward.
            mcp_inputs = tuple(x for x in hidden_states if x is not None)
            def _mcp_recompute(*xs):
                rebuilt = []
                it = iter(xs)
                for h in hidden_states:
                    rebuilt.append(next(it) if h is not None else None)
                return self.three_level_mcp(
                    hidden_states=tuple(rebuilt),
                    vis_span=vis_span,
                    ir_span=ir_span,
                    txt_span=txt_span,
                )[0]
            fusion_cond = torch.utils.checkpoint.checkpoint(
                _mcp_recompute, *mcp_inputs, use_reentrant=False
            )
            # Logging-only weights are intentionally detached to avoid retaining
            # another graph through the checkpointed MCP.
            weight_info = {"level_weight": fusion_cond.new_tensor([[1.0/3, 1.0/3, 1.0/3]])}
        else:
...
def sample_fused_image(
    ...
    num_steps: int,
    device: str,
    sana_dtype: torch.dtype = torch.float32,
) -> torch.Tensor:
    """
    使用与训练阶段一致的 SANA 精度进行采样。
    采样阶段重点：
      1. 不再硬编码 FP16；
      2. 每个 scheduler step 检查 latent 是否出现 NaN/Inf；
      3. 对异常 latent 在 VAE 前进行有限性保护；
      4. VAE 使用自身参数 dtype。
    """
    ...
    # 采样 condition 保持训练阶段相同的数值范围
    condition = condition.float()
    rms = torch.sqrt(
        torch.mean(condition * condition, dim=-1, keepdim=True) + 1e-6
    )
    condition = condition / rms
    condition = torch.clamp(condition, -8.0, 8.0)
    condition = condition.to(sana_dtype)
    ...
    for step_idx, t in enumerate(scheduler.timesteps):
        ...
        if not torch.isfinite(latents).all():
            logger.error(
                f"采样第 {step_idx} 步进入 SANA 前 latent 已经 NaN/Inf"
            )
        ...
        elif sana_dtype == torch.bfloat16:
            ctx = torch.autocast(
                device_type="cuda",
        ...
        # scheduler 在 FP32 做更新
        pred = pred.float()
        old_latents = latents.float()

        if not torch.isfinite(pred).all():
            logger.error(
                f"采样第 {step_idx} 步 SANA pred 出现 NaN/Inf"
            )
        ...
        if not torch.isfinite(latents).all():
            logger.error(
                f"采样第 {step_idx} 步 scheduler 输出 NaN/Inf，"
                f"执行有限性恢复"
            )
        ...
        # 防止异常 step 造成下一步爆炸
        latents = torch.clamp(latents, -20.0, 20.0)

        latents = latents.to(sana_dtype)
    ...
    if vae_device != torch.device(device):
        vae.to(device)
        vae_device = torch.device(device)

    # 当前版本 tiled_decode 有 stride 兼容问题，
    # 单张 16x16 latent 的采样直接关闭 tiled decode。
    was_tiling = bool(getattr(vae.config, "use_tiling", False))

    if was_tiling and hasattr(vae, "disable_tiling"):
        vae.disable_tiling()

    try:
        decode_input = latents.to(
            device=vae_device,
            dtype=vae_dtype,
        )
    ...
    if not torch.isfinite(decoded).all():
        raise FloatingPointError(
            "VAE decode 输出 NaN/Inf。"
            "VAE 输入虽然有限，但 VAE 精度可能存在数值溢出。"
        )
...
def _check_finite(name: str, x: torch.Tensor, *, raise_on_bad: bool = True):
    """训练早期定位 NaN/Inf，避免把数值问题误判为优化器或显存问题。"""
    ok = torch.isfinite(x).all().item()
    if not ok:
        finite = torch.nan_to_num(x.detach().float(), nan=0.0, posinf=0.0, neginf=0.0)
        logger.error(
            f"数值异常: {name} 含 NaN/Inf | "
            f"shape={tuple(x.shape)} | "
            f"finite_min={finite.min().item():.4e} | "
            f"finite_max={finite.max().item():.4e} | "
            f"finite_mean={finite.mean().item():.4e}"
        )
        if raise_on_bad:
            raise FloatingPointError(f"{name} contains NaN/Inf")


def _stabilize_condition_for_dit(condition: torch.Tensor, target_dtype: torch.dtype = torch.float16) -> torch.Tensor:
    """
    FP32 MCP -> SANA FP16 前做有限性检查 + token-wise RMS normalization + clamp。
    不修改 conditioner 参数本身，只稳定送入 SANA 的条件，梯度仍可回传。
    """
    _check_finite("condition_before_dit", condition)
    c = condition.float()
    rms = torch.sqrt(torch.mean(c * c, dim=-1, keepdim=True) + 1e-6)
    c = c / rms
    c = torch.clamp(c, -8.0, 8.0)
    _check_finite("condition_for_dit", c)
    return c.to(target_dtype)


def _select_sana_dtype(sana_dtype_arg: str = "fp32"):
    """Choose a numerically stable SANA dtype for the local GPU."""
    # BF16 has much wider exponent range than FP16, but many 6GB Pascal/Turing
    # cards do not support native BF16. Fall back to FP32 for numerical safety.
    if sana_dtype_arg == "bf16":
        if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
            return torch.bfloat16
        logger.warning("当前 GPU/PyTorch 不支持 BF16，回退到 FP32 SANA")
        return torch.float32
    if sana_dtype_arg == "fp16":
        return torch.float16
    return torch.float32


def _log_cuda_memory(tag: str):
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated() / 1024**3
        reserved = torch.cuda.memory_reserved() / 1024**3
        peak = torch.cuda.max_memory_allocated() / 1024**3
        logger.info(
            f"[CUDA:{tag}] allocated={allocated:.2f}GB "
...
def _scan_gradients(module: nn.Module, prefix: str = "grad"):
    """返回 (finite, norm, bad_info)，并尽量指出异常梯度来自哪个参数。"""
    bad = []
    sq_norm = 0.0
    max_abs = 0.0
    names = []
    for name, p in _named_trainable_params(module):
        g = p.grad
        if g is None:
            continue
        gf = g.detach().float()
        if not torch.isfinite(gf).all():
            bad.append((name, tuple(g.shape)))
            names.append(name)
            continue
        sq_norm += float(torch.sum(gf * gf).item())
        if gf.numel():
    ...
    if bad:
        ...
        logger.error(
            f"[{prefix}] 非有限梯度: tensors={len(bad)} "
            f"preview={preview}"
        )
        return False, float("nan"), bad
    return True, norm, []


def _clone_grads(module: nn.Module):
    """保存当前累积梯度；参数数量只有 1.36M，额外约 5.5MB FP32。"""
    saved = {}
    for name, p in _named_trainable_params(module):
        saved[name] = None if p.grad is None else p.grad.detach().clone()
    return saved


def _restore_grads(module: nn.Module, saved):
    for name, p in _named_trainable_params(module):
        g = saved.get(name)
        if g is None:
            p.grad = None
...
def _check_and_clip_gradients(module: nn.Module, max_norm: float) -> tuple[bool, float]:
    """最终 optimizer.step 前的梯度有限性检查与裁剪。"""
    params = [p for p in module.parameters() if p.requires_grad and p.grad is not None]
    ok, grad_norm, bad = _scan_gradients(module, prefix="optimizer")
    if not ok:
        return False, float("nan")
    total_norm = torch.nn.utils.clip_grad_norm_(params, max_norm, error_if_nonfinite=True)
    return True, float(total_norm.detach().float().item())
...
def train(args):
    ...
    conditioner.use_mcp_checkpoint = getattr(args, "mcp_checkpoint", True)
    ...
    sana_dtype = _select_sana_dtype(getattr(args, "sana_dtype", "fp32"))
    logger.info(f"SANA 数值精度: {sana_dtype}")
    ...
    vae = AutoencoderDC.from_pretrained(
        ...
        # Pixel backward previously produced NaN through the fp16 VAE decoder.
        # Keep the VAE weights FP32 for numerical stability; it lives on CPU
        # except for the low-resolution pixel-loss/visualization phase.
        torch_dtype=torch.float32, low_cpu_mem_usage=True, use_safetensors=True,
    ).to(device)
    vae.requires_grad_(False)
    vae.eval()
    # AutoencoderDC 支持 tiled decode，可显著降低解码峰值显存。
    if hasattr(vae, "enable_tiling"):
        # 当前 diffusers/AutoencoderDC 版本在自定义 stride=0.5 时会把
        # tile_latent_stride_* 计算成 float，tiled_decode 的 range() 随即报错。
        # 这里使用官方默认 tiling 配置，避免该版本兼容性问题。
        try:
            vae.enable_tiling()
            logger.info("VAE tiled decode 已启用（默认整数 stride 配置）")
        except Exception as e:
            logger.warning(f"VAE tiled decode 启用失败，将使用普通 decode: {e}")
    scaling_factor = float(getattr(vae.config, "scaling_factor", 1.0))
    logger.info(f"VAE 数值精度: {next(vae.parameters()).dtype}")

    # 训练阶段 VAE 只在像素损失/采样时短暂驻留 GPU，避免与 FP32 SANA 同时占满 6GB。
    vae.to("cpu")
    gc.collect()
    torch.cuda.empty_cache()
    _log_cuda_memory("models_loaded_sana_only")
    ...
    # 真正的梯度累积：不能在每个 sample 开始时 zero_grad。
    ...
    while global_step < total_steps:
        ...
        for sample in loader:
            ...
            # 保存进入本样本前的累计梯度；若本样本 backward 出现 NaN，恢复它。
            pre_sample_grads = _clone_grads(conditioner)
            ...
            latents = sample["target_latent"].to(device, dtype=torch.float16)
            ...
            noisy_latents = ((1.0 - sigmas) * latents.float() + sigmas * noise.float()).to(torch.float16)
            ...
            condition_dit = _stabilize_condition_for_dit(condition, target_dtype=sana_dtype)
            # SANA 是冻结的，但其输入需要梯度，因此不能放入 no_grad。
            # FP32 模式牺牲部分速度换取 6GB 卡上可控且稳定的数值范围。
            if sana_dtype == torch.float16:
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    diffusion_pred = dit(
                        hidden_states=noisy_latents.to(torch.float16),
                        timestep=timesteps,
                        encoder_hidden_states=condition_dit,
                        encoder_attention_mask=cond_mask,
                    ).sample.float()
            elif sana_dtype == torch.bfloat16:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    diffusion_pred = dit(
                        ...
                        timestep=timesteps,
                        encoder_hidden_states=condition_dit.to(torch.bfloat16),
                        encoder_attention_mask=cond_mask,
                    ).sample.float()
            else:
                diffusion_pred = dit(
                    ...
                    timestep=timesteps,
                    encoder_hidden_states=condition.float(),
                    encoder_attention_mask=cond_mask,
                ).sample.float()
            del condition_dit
            _check_finite("diffusion_pred", diffusion_pred)

            # SANA 不再需要继续占用 VAE 的显存；像素损失前再把 VAE 搬回 GPU。
            ...
            _check_finite("diff_loss", diff_loss)

            x1_hat = noisy_latents.float() - sigmas * diffusion_pred
            _check_finite("x1_hat", x1_hat)

            # 6GB 显卡的关键优化：
            # 不在训练损失中每步完整解码 512x512，而是在更低分辨率 latent 上解码。
            # 仍保持梯度：pool(x1_hat) -> VAE -> pixel_loss -> condition。
            pixel_latent_size = int(args.pixel_latent_size)
            if x1_hat.shape[-1] != pixel_latent_size:
                x1_pixel = F.interpolate(
                    x1_hat,
                    size=(pixel_latent_size, pixel_latent_size),
                    mode="bilinear",
                    align_corners=False,
                )
            else:
            ...
            vae.to(device)
            if hasattr(vae, "enable_tiling"):
                # 训练阶段低分辨率 latent；默认 decode 更兼容。
                try:
                    vae.disable_tiling()
                except Exception:
                    pass
            # VAE 参数为 FP32，输入也保持 FP32；禁止 autocast/FP16，避免
            # decoder backward 的 half-precision overflow。
            with torch.autocast(device_type="cuda", enabled=False):
                decoded = torch.utils.checkpoint.checkpoint(
                    lambda z: vae.decode(z).sample,
                    (x1_pixel / scaling_factor).float(),
                    use_reentrant=False,
                )
            fused_img = ((decoded.float() + 1.0) / 2.0).clamp(0, 1)

            vis_img = sample["vis_img"].to(device, dtype=torch.float32)
            ir_img = sample["ir_img"].to(device, dtype=torch.float32)

            # 与低分辨率 VAE 输出对齐后计算像素融合损失。
            loss_h, loss_w = fused_img.shape[-2:]
            vis_img_loss = F.interpolate(
                vis_img, size=(loss_h, loss_w), mode="bilinear", align_corners=False
            )
            ir_img_loss = F.interpolate(
                ir_img, size=(loss_h, loss_w), mode="bilinear", align_corners=False
            )

            pixel_loss, pixel_parts = pixel_loss_fn(
                fused_img.float(), vis_img_loss, ir_img_loss
            )

            del x1_pixel, decoded, fused_img, vis_img_loss, ir_img_loss

            # ---- 7. 分离 backward：先 flow，再 pixel ----
            # 目的：如果 VAE/pixel 路径的反向产生 NaN，只回退本样本的 pixel 梯度，
            # 保留 flow matching 对条件模块的稳定梯度。
            total_loss = diff_loss + args.pixel_weight * pixel_loss
            scale = float(args.grad_accum)

            # 7a. flow gradient
            (diff_loss / scale).backward(retain_graph=True)
            flow_ok, flow_norm, _ = _scan_gradients(conditioner, prefix="flow_grad")

            if not flow_ok:
                _restore_grads(conditioner, pre_sample_grads)
                accumulation_count += 1
                logger.warning(
                    f"[backward] step={global_step} flow backward 非有限，"
                    "跳过本样本；保留已有累计梯度"
                )
            else:
                # 保存加入 flow 后的累计梯度，作为 pixel 反向失败时的回滚点。
                flow_snapshot = _clone_grads(conditioner)

                # 7b. pixel gradient
                try:
                    (args.pixel_weight * pixel_loss / scale).backward()
                    pixel_ok, pixel_norm, _ = _scan_gradients(conditioner, prefix="pixel_grad")
                except RuntimeError as exc:
                    pixel_ok = False
                    pixel_norm = float("nan")
                    logger.error(f"[pixel_grad] backward RuntimeError: {exc}")

                if not pixel_ok and args.pixel_grad_fallback:
                    # 只丢弃当前样本 pixel 梯度，不污染累计的 flow 梯度。
                    _restore_grads(conditioner, flow_snapshot)
                    logger.warning(
                        f"[backward] step={global_step} pixel backward 非有限，"
                        "回退到 flow-only 梯度"
                    )
                elif not pixel_ok:
                    raise FloatingPointError("pixel backward contains NaN/Inf")

                del flow_snapshot

            accumulation_count += 1

            # VAE 梯度已经回传完毕，现在可以安全搬回 CPU。
            vae.to("cpu")
            gc.collect()
            torch.cuda.empty_cache()

            # ---- 8. 梯度累积步 ----
            if accumulation_count >= args.grad_accum or (global_step + 1) == total_steps:
                grads_ok, grad_norm = _check_and_clip_gradients(
                    conditioner, args.max_grad_norm
                )
                ...
                if grads_ok:
                    ...
                    param_bad = any(
                        ...
                        for p in conditioner.parameters() if p.requires_grad
                    )
                    if param_bad:
                        raise FloatingPointError(
                            "optimizer.step() 后 conditioner 参数出现 NaN/Inf；"
                            "请降低学习率或检查异常梯度。"
                        )
                    logger.info(
                        f"[optimizer] step={global_step} grad_norm={grad_norm:.4e} "
                        f"lr={optimizer.param_groups[0]['lr']:.3e}"
                    )
                else:
                    logger.warning(
                        f"[optimizer] step={global_step} 累计梯度仍非有限，跳过更新"
                    )
            ...
            if global_step % args.log_every == 0:
                ...
                lw = weight_info["level_weight"][0].detach().float().cpu().tolist()
                logger.info(
            ...
            if args.sample_every > 0 and (
                ...
                conditioner.eval()
                try:
                    logger.info("采样阶段：临时将 VAE 从 CPU 搬到 CUDA")
                    fused = sample_fused_image(
                        dit, vae, sample_scheduler,
                        _stabilize_condition_for_dit(condition.detach(), target_dtype=sana_dtype), args.sample_steps, device,sana_dtype=sana_dtype,
                    )
...
def parse_args():
    ...
    p.add_argument("--pixel_grad_fallback", action="store_true", default=True,
                   help="pixel backward 出现 NaN 时回退到 flow-only 梯度；默认开启")
    ...
    p.add_argument("--sana_dtype", type=str, default="fp32", choices=["fp16", "bf16", "fp32"],
                   help="SANA 数值精度；6GB GPU 默认 fp32 以避免 FP16 NaN")

    # 损失权重
    p.add_argument("--pixel_latent_size", type=int, default=4,
                    choices=[2, 4, 8, 16],
                    help="训练时像素损失使用的 latent 边长；4 对应约 128x128 FP32-VAE 解码，显存/稳定性折中")
    ...
    p.add_argument("--no_gradient_checkpointing", dest="gradient_checkpointing",
                   action="store_false")
    p.add_argument("--mcp_checkpoint", action="store_true", default=True,
                   help="对 ThreeLevelFusionMCP 使用 activation checkpoint（6GB GPU 建议保持开启）")
    p.add_argument("--no_mcp_checkpoint", dest="mcp_checkpoint", action="store_false")
