...
def parse_args():
    ...
    parser.add_argument("--chunk-index", type=int, default=None, 
                        help="Index of current chunk (0-based)")
...
def load_models(args):
    ...
    clip_model, _, transform = open_clip.create_model_and_transforms(clip_arch, pretrained="openai", device=DEVICE, cache_dir="/proj/cvl/users/x_fahkh2/temp_cache_geneval")
...
def get_folders_to_process(args):
    """Get list of folders to process, optionally filtered by chunk"""
    all_folders = []
    for subfolder in sorted(os.listdir(args.imagedir)):
        folderpath = os.path.join(args.imagedir, subfolder)
        if os.path.isdir(folderpath) and subfolder.isdigit():
            all_folders.append(subfolder)
    
    # If chunking is specified, filter to only this chunk's folders
    if args.chunk_index is not None and args.n_chunks is not None:
        total_folders = len(all_folders)
        chunk_size = total_folders // args.n_chunks
        remainder = total_folders % args.n_chunks
        
        # Calculate start and end indices for this chunk
        if args.chunk_index < remainder:
            # First 'remainder' chunks get an extra folder
            start_idx = args.chunk_index * (chunk_size + 1)
            end_idx = start_idx + chunk_size + 1
        else:
            # Remaining chunks get the standard size
            start_idx = remainder * (chunk_size + 1) + (args.chunk_index - remainder) * chunk_size
            end_idx = start_idx + chunk_size
        
        folders_to_process = all_folders[start_idx:end_idx]
        
        print(f"Chunk {args.chunk_index}/{args.n_chunks}: Processing folders {start_idx}-{end_idx-1} "
              f"({len(folders_to_process)} folders out of {total_folders} total)", 
              file=sys.stderr)
    else:
        folders_to_process = all_folders
        print(f"Processing all {len(folders_to_process)} folders", file=sys.stderr)
...
def main(args):
    ...
    for subfolder in folders_to_process:
        ...
        if not os.path.exists(metadata_file):
            print(f"Warning: No metadata.jsonl in {folderpath}, skipping", file=sys.stderr)
            continue
        ...
        if not os.path.exists(samples_dir):
            print(f"Warning: No samples directory in {folderpath}, skipping", file=sys.stderr)
            continue
    ...
    print(f"Processed {len(full_results)} images from {len(folders_to_process)} folders", 
          file=sys.stderr)
